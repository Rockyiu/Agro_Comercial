...
class SyncService {
  ...
  Future<void> _syncTransactionsFromServer() async {
    ...
    final parsedTransactionsFromServer = List.from(
      transactionsFromServerResponse['transaction'] ?? [],
    );
    ...
    clock.stop();
    log(
      'total sync time: ${clock.elapsed.inSeconds}',
  ...
  Future<void> _syncBalanceFromServer() async {
    ...
    final localBalanceResponse = (await databaseService.read(
      path: TransactionRepository.balancesPath,
    ));

    if ((localBalanceResponse['data'] as List).isNotEmpty) return;

    final remoteBalanceResponse = await graphQLService.read(
      path: Queries.qGetBalances,
    );
  ...
  Future<void> saveLocalChanges({
    ...
    final response = await databaseService.create(path: path, params: params);

    if (!(response['data'] as bool)) {
      throw const CacheException(code: 'write');
    }

    await secureStorageService.write(key: 'NEED_SYNC', value: true.toString());
  }
    ...
    try {
      ...
      switch (localTransaction.syncStatus) {
        ...
        case SyncStatus.update:
          ...
          transactionWithoutUserId.removeWhere(
